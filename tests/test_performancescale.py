...
def scale(mock_logger, mock_window):
    ...
    with patch(
        ...
        ps = Performancescale("perf", container, level_min=0, level_max=100, tick_number=5, color=C["GREEN"])
...
class TestDrawScale:
    ...
    def test_different_params_produce_different_labels(self, mock_logger):
        ...
        with patch(
            ...
            container = Container("test", 100, 100, 200, 400)
            ps = Performancescale("perf", container, level_min=0, level_max=10, tick_number=3, color=C["GREEN"])
...
class TestSetLevelMin:
    ...
    def test_changes_level_min(self, scale):
        ...
        tick_labels = sorted(k for k in scale.vertex if k.startswith("tick_") and k.endswith("_label"))
        assert tick_labels == sorted(
            ["tick_20_label", "tick_40_label", "tick_60_label", "tick_80_label", "tick_100_label"]
        )
...
class TestSetLevelMax:
    ...
    def test_changes_level_max(self, scale):
        ...
        tick_labels = sorted(k for k in scale.vertex if k.startswith("tick_") and k.endswith("_label"))
        assert tick_labels == sorted(
            ["tick_0_label", "tick_50_label", "tick_100_label", "tick_150_label", "tick_200_label"]
        )
