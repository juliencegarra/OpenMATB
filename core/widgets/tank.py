...
class Tank(AbstractWidget):
    def __init__(self, name, container, letter, level, fluid_label, level_max, target, toleranceradius, infoside):
    ...
    def get_tolerance_vertices(self, radius, target_level, level_max):
        t_width = 15
        t_left = self.container.l - t_width if self.infoside == "left" else self.container.l + self.container.w
