...
class PlayPause(Button):
    ...
    def __init__(self, name, container, callback):
        ...
        self.add_vertex('play_tri', 3, GL_TRIANGLES, g,
                        ('v2f/static', (cx - 0.3 * s, cy + 0.5 * s,
                                        cx - 0.3 * s, cy - 0.5 * s,
                                        cx + 0.5 * s, cy)),
        ...
        yt, yb = cy + bh, cy - bh
        self.add_vertex('pause_bars', 8, GL_QUADS, g,
                        ('v2f/static', (lx1, yt, lx2, yt, lx2, yb, lx1, yb,
                                        rx1, yt, rx2, yt, rx2, yb, rx1, yb)),
...
class MuteButton(Button):
    ...
    def __init__(self, name, container, callback):
        ...
        self.add_vertex('spk_body', 4, GL_QUADS, g,
                        ('v2f/static', (bx1, byt, bx2, byt, bx2, byb, bx1, byb)),
                        ('c4B/static', W * 4))

        # --- Speaker cone (quad / trapezoid) ---
        tip_x = cx + 0.2 * s
        self.add_vertex('spk_cone', 4, GL_QUADS, g,
                        ('v2f/static', (bx2, byt, tip_x, cy + 0.55 * s,
                                        tip_x, cy - 0.55 * s, bx2, byb)),
                        ('c4B/static', W * 4))

        # --- X mark (2 lines = 4 vertices) — visible when muted ---
        xx1, xx2 = cx + 0.35 * s, cx + 0.85 * s
        xy1, xy2 = cy + 0.45 * s, cy - 0.45 * s
        self.add_vertex('mute_x', 4, GL_LINES, g,
        ...
        self.add_vertex('unmute_waves', n_wave_pts, GL_LINES, g,
    ...
    def update_mute_state(self, is_muted):
        ...
        self.on_batch['unmute_waves'].colors = (list(HIDDEN * self._n_wave_pts) if is_muted
                                                else list(W * self._n_wave_pts))
